...
/* ── Hero: track if zoom overlay is active ── */
let zoomActive   = false; /* true while the black overlay is covering screen */
let zoomComplete = false; /* true once zoom has fired once — never fires again */
...
function drawSmoke(W, H) {
  ...
  for (let i = particles.length - 1; i >= 0; i--) {
    const p = particles[i];
    p.x += p.vx + Math.sin(waveT*2+i*.4)*.25;
    p.y += p.vy; p.r += .05; p.life -= p.decay;
...
zoomOverlay.style.cssText='position:fixed;inset:0;background:#060608;z-index:250;pointer-events:none;opacity:0;transition:opacity .0s;';
...
function triggerZoom() {
  ...
  setTimeout(() => {
    /* 2. Jump scroll to ADN — user is behind black overlay so no jank */
...
function loop() {
  ...
  /* Zoom — only if not yet done and hero is nearly at end */
  if (!zoomComplete) {
    const zoomP = Math.max(0, Math.min((hp - 0.55) / 0.30, 1));
    if (zoomP > 0) {
      const scale   = 1 + Math.pow(zoomP, 2.5) * 14;
